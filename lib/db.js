...
export async function sluggify(name, table, column, searchLimit="100") {
    let slug = name.replace(/ /g,"-");

    let i = 0;
    while (i < searchLimit) {
        var slugTaken = await new Query(table, column).select().where(column + " = ?", slug).run();

        if (slugTaken === undefined) {
            return null;
        }

        console.log(slugTaken)

        if (slugTaken === null || slugTaken == []) {
            return slug;
        }
