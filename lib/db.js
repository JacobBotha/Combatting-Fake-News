...
import { insertQuestion } from './quiz';
...
export const tables = {
    ...
    quizQuestion: 'QUIZ_HAS_QUESTIONS' 
};
...
export class Query {
     ...
     * @returns {Query} - Returns this.
     */
    innerJoin(joinTable, column, foreignKey, conditions, 
     ...
     * @returns {Query} - Returns this.
     */
     leftJoin(joinTable, column, foreignKey, conditions, 
        conditionSeperator = " AND ") {
        var joinType = " LEFT "
        let statement = this._join(joinTable, column , foreignKey, conditions,
            conditionSeperator, joinType);
    ...
    _join(joinTable, column , foreignKey, conditions, conditionSeperator, 
        ...
        this.table + "." + column + " = " + joinTable + "." + foreignKey +
        conditions;
    ...
    _valuesString(values) {
        ...
        if (values instanceof Array) {
            ...
            throw Error("Incoorect Type passed as values, only string and string[]" +
            "allowed");
        }
    ...
    _conditionSeperator(conditions, conditionSeperator = " AND ") {
        if (conditions instanceof Array) {
            return conditions.join(conditionSeparator);
